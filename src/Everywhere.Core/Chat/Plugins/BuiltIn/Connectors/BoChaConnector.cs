...
public sealed partial class WebBrowserPlugin
{
    private partial class BoChaConnector : IWebSearchEngineConnector
    {
        ...
        public async Task<IEnumerable<T>> SearchAsync<T>(string query, int count = 1, int offset = 0, CancellationToken cancellationToken = default)
        {
            ...
            var response = JsonSerializer.Deserialize(json, BoChaResponseJsonSerializationContext.Default.BoChaResponse);
        ...
        private partial class BoChaResponseJsonSerializationContext : JsonSerializerContext;

        private class BoChaResponse
        {
            ...
            public BoChaResponseData? Data { get; init; }
        }

        private sealed class BoChaResponseData
        {
