...
public sealed class OfficeCLIPlugin : BuiltInChatPlugin
{
    ...
    [Description(
        ...
        For any of the commands in the following instructions, do not use any other tools; you MUST use THIS tool and pass the `commandLine` parameter to execute them.
    ...
    private async Task<string> ExecuteAsync(string commandLine, CancellationToken cancellationToken)
    {
        using var process = Process.Start(
            new ProcessStartInfo
            {
                FileName = _executablePath,
                Arguments = commandLine.TrimStart("officecli"),
        ...
        if (output.IsNullOrEmpty() && error.IsNullOrEmpty())
        {
        ...
        return output.IsNullOrEmpty() ? error : output;
    }
}